...
<form action="<?= esc(base_url('user/ubahBarangIT')); ?>" method="post" role="form">
    <?= csrf_field() ?>
    ...
    <div class="row form-group mb-3">
        ...
        <div class="col-7">
            <select class="form-control selectpicker" name="user_id" value=""  data-live-search="true" data-size="5" data-style="" data-style-base="form-control" required>
                <option disabled value="">Pilih Users</option>
        ...
        <div class="col-7">
            ...
            <datalist id="list_merk">
                ...
                <?php foreach ($merk_list as $merk) : ?>
                    <option class="merk_list" <?= $barang['merk'] == $merk ? ' selected="selected"' : ''; ?>><?= esc($merk['merk']) ?></option>
                <?php endforeach; ?>
            </datalist>
    ...
    <div class="row form-group mb-3">
        ...
        <div class="col-7">
            ...
            <select class="form-control selectpicker" name="lokasi" value="<?= $barang['lokasi'] ?>"  data-live-search="true" data-size="4" data-live-search-placeholder="Pilih Lokasi Barang" data-style="" data-style-base="form-control" required>
                <!--<option class="text-center" value="" disabled>Pilih Lokasi Barang</option>-->
    ...
    <input name="url" type="text" class="d-none" value="<?= base_url('user/kelola_barangit'); ?>">
    <div class="row form-group mb-3 offset-5">
        <div class="col-auto">
            <a href='<?= base_url('user/kelola_barangit') ?>' class="btn btn-sm btn-sm btn-secondary">Kembali</a>
        </div>
        <div class="col-auto">
...
<script>
    $(document).ready(function() {
        $('select[name=jenis]').selectpicker('val','<?=$barang['jenis']?>');
         $('select[name=user_id]').selectpicker('val','<?=$barang['user_id']?>');
        $('select[name=lokasi]').selectpicker('val','<?=$barang['lokasi']?>');
        ...
        $.ajax({
            ...
            .done(function(msg) {
                ...
                msg.data.forEach(function(item, index) {
                    ...
                    $("#tipe_select").append('<option ' + select_ + ' class="tipe_list">' + item.tipe + '</option>');
                });
